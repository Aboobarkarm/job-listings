  ...
  return (
    <nav className='fixed top-0 left-0 right-0 z-40 bg-indigo-700 border-b border-indigo-500'>
       ...
       <button className='md:hidden flex items-center text-white' onClick={toggleSidebar}>
        ...
        <div className={`md:flex md:items-center md:space-x-6 ${isSidebarOpen ? 'block' : 'hidden'} text-white fixed md:static top-0 right-0 h-50 border border-indigo-500 md:border-none rounded-lg shadow-sm md:h-auto w-[150px] md:w-auto bg-indigo-700 md:bg-transparent p-5 md:p-0 transform ${isSidebarOpen ? 'translate-x-0' : 'translate-x-full'} md:transform-none transition-transform duration-300 ease-in-out`}>
          ...
          <Link href="/add-job" className={`block tracking-wider mt-4  md:mt-0 ${pathname === '/add-job' ? 'font-bold' : ''}`}>
            <div className='flex items-start justify-start'>
